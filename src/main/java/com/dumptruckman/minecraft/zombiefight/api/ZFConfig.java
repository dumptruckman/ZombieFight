...
public interface ZFConfig extends BaseConfig {
    ...
    ConfigEntry<Integer> MIN_PLAYERS = new EntryBuilder<Integer>(Integer.class, "game_settings.players.minimum_players").stringSerializer().def(15).defaultIfMissing().comment("# The minimum players required to start a game.").build();
    ...
    ListConfigEntry<Integer> COUNTDOWN_WARNINGS = new EntryBuilder<Integer>(Integer.class, "game_settings.times.start_countdown.warnings").defList(Arrays.asList(60, 30, 15, 10, 5)).defaultIfMissing().stringSerializer().comment("# The seconds at which a countdown warning will display.").buildList();
    ConfigEntry<Integer> END_DURATION = new EntryBuilder<Integer>(Integer.class, "game_settings.times.end_game_duration").stringSerializer().def(30).defaultIfMissing().comment("# How long to wait (in seconds) after a game has finished before beginning a new game.").build();
    ConfigEntry<Integer> FINAL_HUMAN = new EntryBuilder<Integer>(Integer.class, "game_settings.times.last_human_duration").stringSerializer().def(30).defaultIfMissing().comment("# How long the last human alive muts survive (in seconds) before they are declared winner.").build();
    ConfigEntry<Integer> ZOMBIE_LOCK = new EntryBuilder<Integer>(Integer.class, "game_settings.times.zombie_lock_duration").stringSerializer().def(30).defaultIfMissing().comment("# The amount of time in seconds that the first zombie will be locked in place and unable to damage players.").build();

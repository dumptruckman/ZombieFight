...
public interface ZFConfig extends BaseConfig {
    ...
    ConfigEntry<Integer> FINAL_HUMAN = new EntryBuilder<Integer>(Integer.class, "game_settings.times.last_human_duration").stringSerializer().def(180).defaultIfMissing().comment("# How long the last human alive muts survive (in seconds) before they are declared winner.").build();
    ConfigEntry<Integer> ZOMBIE_LOCK = new EntryBuilder<Integer>(Integer.class, "game_settings.times.zombie_lock_duration").stringSerializer().def(60).defaultIfMissing().comment("# The amount of time in seconds that the first zombie will be locked in place and unable to damage players.").build();

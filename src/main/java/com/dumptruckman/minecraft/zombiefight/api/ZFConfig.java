...
public interface ZFConfig extends BaseConfig {
    ...
    MappedConfigEntry<Integer> BORDER_RADIUS = new EntryBuilder<Integer>(Integer.class, "data.border.radius").def(250).defaultIfMissing().stringSerializer().comment("# The radius of the border from the game spawn.").buildMap();
    ...
    ConfigEntry<Integer> MIN_PLAYERS = new EntryBuilder<Integer>(Integer.class, "game_settings.players.minimum_players").stringSerializer().def(4).defaultIfMissing().comment("# The minimum players required to start a game.").build();
    ...
    ConfigEntry<Integer> ZOMBIE_DAMAGE = new EntryBuilder<Integer>(Integer.class, "game_settings.zombie.damage_bonus").stringSerializer().def(3).defaultIfMissing().comment("# This is the amount of extra damage zombies do.").build();

...
@CommandInfo(
        primaryAlias = "select",
        desc = "Selects the active game world.",
        flags = "w"
)
public class SelectCommand extends ZFCommand {
    ...
    @Override
    public Perm getPerm() {
        return Perms.CMD_SELECT;
    }

    @Override
    public Message getHelp() {
        return Language.CMD_SELECT_HELP;
    }
    ...
    public void runCommand(ZombieFight p, BasePlayer sender, CommandContext context) {
        ...
        if (!p.getGameManager().isWorldEnabled(world)) {
            p.getMessager().bad(sender, Language.CMD_SELECT_NOT_ENABLED);
            return;
        }
        p.getGameManager().setPrimaryGame(world);
        p.getMessager().good(sender, Language.CMD_SELECT_SUCCESS, world.getName());
    }
}
